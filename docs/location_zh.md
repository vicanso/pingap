...
可按需配置该location对应的请求头，两个配置的方式均比较简单一种是直接添加，一种是设置（若已存在则覆盖），需要注意的是，有些`upstream`是需要匹配对应的`Host`的，有此要求的则需要在请求头中设置对应的配置`Host:xxx`(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)(使用设置而非添加)。
